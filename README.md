...
> 放入 Word 策划文档，其余全自动。支持任意 H5 (HTML5 Canvas/WebGL) 手机端 2D 游戏。
...
用 VSCode 打开项目文件夹，点击侧边栏 Claude 图标，复制以下内容发送：
...
Agent 06：读取并执行 .claude/agents/06_developer.md
  → 循环实现 backlog 中所有 pending 任务，每次写入 .ts 文件后立即执行：
    cd web_project && npx tsc --noEmit
    有 ERROR 则停止修复，通过后继续

Agent 07：读取并执行 .claude/agents/07_tester.md
  → 编写并运行 Vitest 测试，修复失败项直到 100% 通过

Agent 08：读取并执行 .claude/agents/08_builder.md
  → 执行 npm run build 构建 H5 产物到 builds/dist/
...
3. 流水线运行后会自动执行 `cd web_project && npm install` 安装依赖

无需 Godot、Android SDK 或其他引擎环境。
...
2. 删除 `tasks/project_context.json`、`tasks/backlog.json`、`web_project/`、`design/systems/`
3. 重新发送启动提示词
...
├── reports/                      自动生成
├── builds/                       自动生成
└── web_project/                  自动生成（Vite + TypeScript 项目）
```
...
**Q：TypeScript 语法检查不通过？**
A：检查 `web_project/tsconfig.json` 配置是否正确，以及是否执行了 `npm install`。流水线会在每个 .ts 文件写入后自动运行 `tsc --noEmit`。
