...
适用于任意 H5 (HTML5 Canvas/WebGL) 手机端 2D 游戏项目。
...
| 06 | 06_developer.md | 功能开发（分批、原子任务、冒烟检查点） | web_project/ |
| 07 | 07_tester.md | 测试补全 + 全量回归（含测试隔离） | reports/test_report.md |
| 08 | 08_builder.md | 构建导出 | builds/ |
...
└── web_project/
    ...
    │   ├── scenes/        ← 场景/页面级模块
    │   ├── systems/       ← 游戏系统逻辑
    │   ├── entities/      ← 游戏实体（精灵、单位等）
    │   ├── ui/            ← UI 组件
    │   ├── data/          ← 静态 JSON 配置
    │   ├── assets/        ← 图片、音频、字体（源码引用）
    │   └── utils/         ← 工具函数
    └── tests/             ← 测试文件（与 Vitest 配合）
```
...
1. 文件命名：`PascalCase.ts`（类文件）或 `camelCase.ts`（模块/工具）
2. 类名：`PascalCase`，使用 `export class` 导出
3. 事件名：`camelCase` 动词过去式（`orderServed`、`dayEnded`），通过 `EventEmitter` 或自定义事件总线派发
4. 常量：`UPPER_SNAKE_CASE`
5. 变量：必须有类型注解（`let gold: number = 0`）
6. 私有成员：以 `_` 开头，并用 `private` 修饰
7. 每个文件顶部必须有注释说明职责
8. 禁止 `console.log()` 调试，用 `console.warn()` / `console.error()`；生产环境应通过构建配置剥离
9. 所有 JSON 读写必须做错误处理（`try/catch` + 类型守卫）
10. 事件订阅优先用代码，避免在 HTML 中内联绑定
11. **`GameData.ts` 必须包含 `resetToDefaults()` 方法**（P2：测试隔离，仅测试环境调用）
...
- 画布适配：Canvas 动态缩放，保持 `contain` 或 `cover` 策略，兼顾安全区
- 触控区域：所有可点击元素最小 **88×88px**（逻辑像素）
- 安全区域：四边各留 **40px**
- 性能：事件驱动 UI 更新，避免每帧在渲染循环中执行复杂逻辑；优先使用 `requestAnimationFrame`
...
# TypeScript 类型检查 / 语法检查（退出码 0 且无 ERROR = 通过）
cd web_project && npx tsc --noEmit 2>&1

# 开发服务器（本地预览）
cd web_project && npm run dev

# 运行 Vitest 测试
cd web_project && npx vitest run 2>&1

# 生产构建（输出到 web_project/dist/）
cd web_project && npm run build 2>&1
```

---

## 强制规则：TypeScript 语法检查

**优先级高于一切其他指令，不可跳过。**

```
写入 .ts 文件
    ↓ 立即执行 cd web_project && npx tsc --noEmit 2>&1
