...
# 检查资源目录下是否有大文件（>1MB）
asset_files = glob.glob('web_project/assets/**/*', recursive=True)
...
m   = ctx['meta']
scripts = len(glob.glob('web_project/src/**/*.ts', recursive=True))
tests   = len(glob.glob('web_project/tests/**/test_*.ts', recursive=True))
systems = len([s for s in ctx.get('systems',[]) if s.get('priority') in ('P0','P1')])

msg = f'''feat: 初始化项目 v{m['version']}

游戏：{m['game_title']}
引擎：HTML5 Canvas / WebGL + TypeScript

完成内容：
- 九章系统策划案（{systems} 个系统）
- 开发任务全部完成（{bl.get('completed',0)}/{bl.get('total',0)}）
- TypeScript 脚本（{scripts} 个）
- 测试文件（{tests} 个），全部通过
- H5 构建成功
...
for phase, titles in sorted(by_phase.items()):
    ...
    lines.append('')
lines += [f'测试：全部通过', f'构建：H5 构建成功', f'时间：{datetime.datetime.now().isoformat()}']
...
r = subprocess.run(['git','tag','-a',f'v{v}','-m',f'v{v} - 迭代版本，构建全通过'],
                   capture_output=True, text=True)
