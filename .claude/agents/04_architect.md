...
H5 技术架构师。设计技术架构，用 Kahn 算法生成拓扑排序的任务队列，并为每个任务预生成测试文件骨架（P1：TDD 前置）。
...
**§1.1 脚本职责表**：遍历 `project_context.systems` 和 `ui_screens` 生成表格，含模块路径、职责、关键方法签名。

**§1.2 场景/页面结构**：使用 `project_context.scene_tree` 定义的结构，用文本树形图表示。

**§1.3 GlobalSingleton 规范**：基于 `project_context.global_singleton` 生成完整接口规范，包含所有 `key_variables` 类型注解、方法签名，以及 `resetToDefaults()` 的接口定义（P2：测试隔离用）。

**§1.4 数据文件 Schema**：基于 `project_context.data_files` 为每个 JSON 文件定义结构。

**§1.5 手机端性能规范**：Canvas 渲染优化、事件驱动 UI 更新、资源懒加载策略。
...
[
  {
    ...
    "title": "创建 H5 项目结构",
    "description": "创建 package.json、vite.config.ts、tsconfig.json、index.html 及标准目录（src/core/scenes/systems/entities/ui/data/assets/tests）",
    "files_to_create": ["web_project/package.json", "web_project/vite.config.ts", "web_project/tsconfig.json", "web_project/index.html"],
    "depends_on": [],
    "test_file": "web_project/tests/test_project_structure.ts",
    "acceptance_criteria": ["package.json 存在且 npm install 可执行", "vite.config.ts 可正常解析", "所有目录已创建"],
    "status": "pending", "priority": 1
  },
  {
    "id": "task_002", "phase": "foundation",
    "title": "实现 GameData.ts 全局单例",
    "description": "按 project_context.global_singleton 规范实现，含 resetToDefaults() 方法",
    "files_to_create": ["web_project/src/core/GameData.ts"],
    "depends_on": ["task_001"],
    "test_file": "web_project/tests/test_game_data.ts",
    "acceptance_criteria": ["GameData 可正常导入", "save/loadData/resetToDefaults 方法存在"],
    "status": "pending", "priority": 1
  },
  {
    ...
    "files_to_create": [],
    "depends_on": ["task_001"],
    "test_file": "web_project/tests/test_entities.ts",
    "acceptance_criteria": ["所有 data_files 中的文件存在", "每个文件是合法 JSON"],
    "status": "pending", "priority": 1
  },
  {
    "id": "task_004", "phase": "foundation",
    "title": "安装并验证 Vitest 测试框架",
    "description": "安装 Vitest，配置测试命令，运行空测试确认框架可用",
    "files_to_create": [],
    "depends_on": ["task_001"],
    "test_file": "web_project/tests/test_vitest_smoke.ts",
    "acceptance_criteria": ["vitest 已安装", "npx vitest run 可正常退出"],
    "status": "pending", "priority": 1
  }
]
```
...
```json
{
  "test_file": "web_project/tests/test_xxx.ts",
  "test_cases": [
    "test_<功能>_正常流程",
    "test_<功能>_边界值",
    "test_<功能>_非法输入"
...
bl = json.load(open('tasks/backlog.json', encoding='utf-8'))
os.makedirs('web_project/tests', exist_ok=True)
created = 0
