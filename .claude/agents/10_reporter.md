...
scripts  = glob.glob('web_project/src/**/*.ts', recursive=True)
tests    = glob.glob('web_project/tests/**/test_*.ts', recursive=True)
data     = glob.glob('web_project/data/**/*.json', recursive=True)
...
stats = {
    'scripts': len(scripts), 'tests': len(tests),
...
**版本**：{stats['version']}  **引擎**：HTML5 Canvas / WebGL  **生成时间**：{datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}
...
| 测试通过率 | 100% |
| 构建状态 | ✅ H5 构建成功（{stats['scripts']} 个模块）|
...
| 功能开发 | 06 | web_project/（{stats['scripts']}个脚本，含冒烟测试）| ✅ |
| 自动测试 | 07 | test_report.md（含测试隔离）| ✅ |
| 构建导出 | 08 | builds/dist/（H5 构建产物）| ✅ |
...
- [ ] 部署到静态服务器 / CDN → 见 [reports/build_guide.md](reports/build_guide.md)
- [ ] 真机浏览器触控测试（最小触控区 88×88px）
...
│   ├── test_results.txt            Vitest 原始输出
│   ├── test_report.md              测试报告（含测试隔离说明）
│   ├── build_report.md             构建报告
│   ├── build_guide.md              Web 部署指南
│   ├── pipeline_log.md             流水线开始/完成日志
│   └── final_report.md             本文件
├── builds/
│   └── dist/                      H5 构建产物
└── web_project/                  完整 H5 项目（Vite + TypeScript）
    ├── src/                        {stats['scripts']} 个 TypeScript 模块
...
print(f'╠{b}╣')
for item in ['📄 reports/final_report.md','🎮 web_project/','📦 builds/dist/',
