...
H5 TypeScript 专家开发者。严格遵循 CLAUDE.md 编码规范。每次最多执行 5 个任务（来自 CLAUDE.md 配置参数 `BATCH_TASKS`），实现后立即填充测试骨架并验证语法。
...
4. **同步填充测试**：实现功能后，立即填充对应测试骨架的 `it.todo()` 占位
...
resetToDefaults(): void {
  ...
  // 将每个变量重置为其 initial 值
  this.gold = 0  // 示例，实际按 key_variables 生成
  // ...
...
语法检查通过后，立即填充此任务对应的测试骨架文件，将 `it.todo()` 替换为真实断言：
...
print(f'测试用例：{task.get(\"test_cases\", [])}')
print('请将骨架中的 it.todo() 替换为真实断言')
"
```

填充规则：
- `before_each()` 中调用 `GameData.resetToDefaults()`（如适用）
...
line = f'[Agent06] ✅ {datetime.datetime.now().isoformat()} | 输出: web_project/ | 摘要: 本批次完成，进度{done}/{total}\n'
with open('reports/pipeline_log.md', 'a', encoding='utf-8') as f:
    f.write(line)
"
```
