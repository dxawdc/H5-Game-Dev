...
**只提取 §1.3（系统关系）、§3.2（数值表）、§6.2（事件列表）、§9.1（MVP）**，不加载全文。
...
**事件一致性（P1）**：
检查 §6.2 中定义的事件名，在被依赖系统中是否有对应的监听描述。
事件名格式必须是 `camelCase` 动词过去式（如 `orderServed`、`dayEnded`）。
...
- [ ] 所有 §6.2 事件名格式正确（`camelCase` 动词过去式）
- [ ] 新手引导流程已定义
- [ ] 全局数值表已汇总
